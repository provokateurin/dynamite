...
TypeResult resolveType(State state, json_schema.JsonSchema schema) {
  final result = _resolveType(state, schema);
...
TypeResult _resolveType(State state, json_schema.JsonSchema schema) {
  switch (schema) {
    case json_schema.JsonSchema($enum: != null):
      ...
      return resolveEnum(state, schema, subResult);
    ...
    case json_schema.JsonSchema(ref: != null):
      final subResult = resolveType(state, schema.resolveRef(state.rootJson));
    ...
    case json_schema.IntegerSchema():
      return TypeResultBase('int', nullable: schema.nullable);

    case json_schema.NumberSchema(format: 'float' || 'double'):
      return TypeResultBase('double', nullable: schema.nullable);

    case json_schema.NumberSchema():
      return TypeResultBase('num', nullable: schema.nullable);

    case json_schema.StringSchema(format: 'binary'):
      return TypeResultBase('Uint8List', nullable: schema.nullable);

    case json_schema.StringSchema():
      return TypeResultBase('String', nullable: schema.nullable);
